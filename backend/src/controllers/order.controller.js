...
async function createOrder(req, res) {
  try {
    const { idPaymentType, idShipmentType, idLocation, shipmentValue } =
      req.body;
...
async function getUserOrders(req, res) {
  try {
    ...
    const result = await orderService.getUserOrdersWithPagination(userId, page, limit);
...
async function getUserOrdersDatails(req, res) {
  try {
    ...
    const result = await orderService.getUserOrdersDetailsService(
      userId,
      page,
      limit
    );
...
async function searchOrders(req, res) {
  try {
    ...
    if (startDate && isNaN(Date.parse(startDate))) {
      return res
        .status(400)
        .json({ message: 'Formato de fecha inicial inválido' });
    }
    if (endDate && isNaN(Date.parse(endDate))) {
      return res
        .status(400)
        .json({ message: 'Formato de fecha final inválido' });
    }
...
async function getActiveOrders(req, res) {
  try {
    ...
    return res
      .status(500)
      .json({ message: 'Error al obtener órdenes activas' });
  }
}
...
async function getOrderHistory(req, res) {
  try {
    ...
    return res
      .status(500)
      .json({ message: 'Error al obtener historial de órdenes' });
  }
}
...
async function reoder(req, res) {
  try {
    ...
    return res.status(200).json({
      message:
        'Pedido reordenado correctamente, productos agregados al carrito',
      data: result,
    });
  } catch (error) {
    ...
    switch (error.message) {
      ...
      case 'Ninguno de los productos de la orden esta disponible actualmente':
        status = 404;
        message =
          'Ninungo de los productos de la orden se encuentran disponible actualmente';
        break;
      case 'La orden no pertenece al usuario':
        status = 403;
        message =
          'No tienes permiso para reordenar este pedido, solo puedes reordenar tus propios pedidos';
        break;
